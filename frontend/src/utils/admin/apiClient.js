 ...
 * Wraps makeAuthenticatedRequest with admin-specific features:
...
/**
 * Make authenticated API request for admin with automatic token refresh
 ...
 * @param {number} config.timeout - Request timeout in milliseconds (default: 30000, 60000 for submissions)
 * @returns {Promise<Response|null>} - Fetch response or null if redirect occurred
 */
export const makeAdminRequest = async (url, options = {}, router = null, config = {}) => {
  // Check for window to avoid SSR errors
  if (typeof window === 'undefined') {
    throw new Error('Cannot make authenticated request on server side');
  }

  // Determine timeout - longer for submissions requests that may contain large data
  const isSubmissionsRequest = url && url.includes('/api/submissions');
  const timeoutMs = config.timeout || (isSubmissionsRequest ? 60000 : 30000);
  ...
  try {
    ...
    if (response.status === 401) {
      ...
      try {
        ...
        logError(parseError, { context: 'makeAdminRequest-parseError' });
      }
    ...
    if (fetchError.name === 'AbortError' || fetchError.message?.includes('aborted')) {
      const timeoutError = new Error('Request timed out. The server is taking too long to respond. This may happen when loading submissions with large data (post-act reports, images). Please try again.');
      timeoutError.status = 408; // Request Timeout
      timeoutError.isTimeout = true;
      logError(timeoutError, { context: 'makeAdminRequest-timeout', url, timeoutMs });
      throw timeoutError;
    }
