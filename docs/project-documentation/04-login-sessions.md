...
3. Backend creates session (same as admin)
4. Frontend stores token in localStorage
5. Session verification (same as admin)

**Code Locations:**
- Backend: `backend/src/superadmin/controllers/superadminController.js` - `loginSuperadmin()`
- Backend: `backend/src/utils/sessionSecurity.js` - Session management
- Backend: `backend/src/utils/twoFA.js` - 2FA implementation

## Session Security Features

1. **Token Expiration**: Short-lived access tokens (15-30 minutes)
2. **Refresh Token Rotation**: Refresh tokens rotated on each use
3. **Session Binding**: IP address and User-Agent fingerprinting
4. **Token Hashing**: Tokens stored as hashes in database
5. **Automatic Cleanup**: Expired sessions automatically removed
6. **Revocation Support**: Ability to revoke sessions
7. **Security Violation Detection**: Sessions revoked on IP/UA mismatch
